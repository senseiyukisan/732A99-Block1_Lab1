...
print(c)
```
* In confusion matrix, rows are taken as actual and the columns are taken as predicted$\bar{Y}$
...
```
According to value We get through the confusion matrix, the miss classification of actual 3 and 4 are high in rate
...
to_binary <- function(i){
 ...
 for (i in 1:30){ 
   ...
   probs<- data.frame(knn.fit$prob)
   max_prob <- colnames(probs)[apply(probs ,1,which.max)]
   probs$target <- validation$X0.26
   probs$fit <- knn.fit$fitted
   probs$max_prob <- max_prob

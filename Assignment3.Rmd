...
The trend is as expected. If lambda is large, the parametsers are heavily constrained and the degrees of freedom will effectively be lower, tending to 0 as $\lambda$ -> $\infty$. For $\lambda$ -> $0$, we have *p* parameters. As in OLS all coefficients stay the same, no penalization to be found.
...
set.seed(12345)
generated_data = rnorm(dim(test_x), test_x, sigma)
...
true_pred_df_ = cbind(test[c(1)], scale(test[c(102)]), generated_data)
colnames(true_pred_df)[2] = "True"
